...
public class MediaPipeRightHandBoneCopying : MonoBehaviour {
    ...
    void Update() {
        if (m_available) {
            // Palm
            Quaternion palmRot = Quaternion.Euler(0f, 0f, 90f) * Quaternion.Euler(-270f, 0f, 0f);
            m_selfFingers[0].position = m_targetFingers[0].position;
            m_selfFingers[0].rotation = m_targetFingers[0].rotation * palmRot;
            //m_selfFingers[0].rotation = m_targetFingers[0].rotation;
            m_selfFingers[0].position += m_selfFingers[0].right * -0.065f;
            m_selfFingers[0].position += m_selfFingers[0].forward * 0.015f;
